...
goals = [1, 1, 6, 6;
         1, 5, 6, 1]; %we have change the order of the goal coordinates because C = [6;6]
goals_name = ['A', 'B', 'C', 'D'];
...
num_episodes = 10000;
epsilon = 1; %the higher the epsilon value, the slower the q-learning process
alpha = 0.1;
gamma = 0.9;
...
mean_rewards_plot = figure(1);
plot(mean_diffs)
title('Averaged max elementwise q diff');
xlabel('episode');
ylabel('mean q diffs');

rewards_plot = figure(2);
plot(rewards)
title('Non averaged rewards');
xlabel('episode');
ylabel('reward');
