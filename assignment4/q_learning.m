...
%% TODO: Implementation
    ...
    for ep = 1:1:num_episodes
        ...
        while ~terminate_episode % as long as episode is not terminated
            ...
            q_table = update_q_table_det(q_table,s_current,a_current,r,s_next,gamma,state_params);%Task2
            %q_table = update_q_table_gen(q_table,s_current,a_current,r,s_next,gamma,state_params,q_visits);%Task6
...
%           consecutive q-tables (between q_table and q_old)
        ...
        % training
        if mod(ep, 100) == 0   %we have moved this lines of code out of the while loop because here we just print the episode once
