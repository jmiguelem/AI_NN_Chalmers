function q_table_out = update_q_table_gen(q_table_in, s_current, a_current, alpha, r, s_next, gamma, state_params,q_visits)
...
% 5 (= 'pick-up') 
% 6 (= 'drop-off') 
%% INPUT(4): alpha (learning rate
% type --> float value between [0, 1]
%% INPUT(5): r (reward generated by a_current)
% type --> int
%% INPUT(6): s_next (next robot state)
% type --> array with 4 values:
...
% type --> float value between [0, 1]
%% INPUT(8): state_params: [grid_size, num_obj_pos, num_goal_pos, num_actions] 
...
%% TODO: Implementation
    ...
    q_table_out(ind_cur,a_current) = q_table_in(ind_cur,a_current) + alpha*(r+gamma*max(q_table_in(ind_next,a_current)-q_table_in(ind_cur,a_current)));
