...
%% TODO: Implementation
    ...
    while hit_obstacle % repeat until you have drawn a row/col which is not an obstacle
        ...
        position = randi(state_params(1),2,1); % Tip: look for the randi function in the documentation to figure out how it works 
        ...
        if ~isempty(obstacles) 
            % TODO
            for i=1:1:length(obstacles) % loop through the obstacles
                if obstacles(1,i)== current_row && obstacles(2,i) == current_col  % check if the init_pos is an obstacle
                    hit_obstacle = 1;
                end
            end
        end
    ...
    pick_and_drop_pos = randi(state_params(3),2,1);
